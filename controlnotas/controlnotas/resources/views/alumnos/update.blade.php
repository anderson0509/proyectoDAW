    ...
    <form action="/alumnos/{{$alumno->codigo}}" method="POST" id="fmrupdateData">
        <div class="row">
            <div class="col">
                <label>NIE</label>
                <input type="text" name="nie" value="{{ $alumno->nie }}" class="form-control">
        ...
        <div class="row">
            <div class="col">
                <label>Correo</label>
                <input type="text" name="correo" value="{{ $alumno->correo }}" class="form-control">
            ...
            <div class="col">
                <label>imagen</label>
                <input type="text" name="imagen" value="{{ $alumno->imagen }}" class="form-control">
            ...
            <div class="col-6">
                ...
                <select name="encargado" class="form-select">
                    ...
                    @foreach ($encargado as $item)
                        <option value="{{ $item->codigo }}" {{($item->codigo == $alumno->encargado)?'selected':''}}>
                            {{ $item->nombre }} {{ $item->apellido }} {{ $item->dui }}
                        </option>
                    @endforeach
                </select>
                <span class="invalid-feedback d-block" key="encargado" role="alert">
                    <strong class="mensaje"></strong>
                </span>
            </div>
        </div>
